...
⚠️ **查最新 tag 必须查内网，不要查本地**：

```bash
git ls-remote --tags iflytek | grep -o 'refs/tags/v[0-9][0-9.]*$' | sed 's|refs/tags/||' | sort -V | tail -1
```

内网是唯一主线，**版本号的真实来源在内网**，本地 tag 只是副本、必然滞后。

三个坑都要避开：
- **查本地 tag 会漏版本**（2026-09-18 真实踩过）：方向反转后本地只打 `vX.Y.Z-iflytek`
  这一个 tag（无后缀的那个只推到内网、本地不留），于是过滤掉 `-iflytek` 后
  **恰好把新版本全滤没了**——当时内网已到 v1.25.1，旧命令却报 v1.25.0，
  照它打就会和已存在的 tag 撞车。故必须 `ls-remote` 查内网。
- 直接 `git tag -l | tail` 是**字符串排序**，会把 `v1.9.0` 排在 `v1.18.0` 后面（真实踩过）
- 正则要用 `v[0-9][0-9.]*$` 而非 `v[0-9.]*`：annotated tag 在 `ls-remote` 里有
  `refs/tags/vX.Y.Z` 和 `refs/tags/vX.Y.Z^{}` 两行，`$` 锚定才能把 `^{}` 那行排除掉
