...
5. **落地前自检** —— 每次改动完成前，逐条核对上述各点；命中「找不到令牌 / 找不到组件 / 想在使用方 scoped 覆盖组件外观 / 引用方接入方式不合规」时必须暂停并向用户说明，由用户决定新增全局定义还是调整方案。**页面 / 模块布局类改动还须输出「样式值对账清单」**（每处新写间距 / 圆角 / 字阶 / 颜色 → 场景锚点，标不出的升级提问，完整要求见 `design-spec/CLAUDE.md` 同条）。
