...
| **界面上出现黑色方框 / 多余描边 / 焦点框相关**（"Windows 上一 hover 就有黑边框""点了按钮留一圈绿框""输入框有内外两圈边""按 Esc 后凭空多出一圈描边""Tab 聚焦没反馈""想去掉这个 outline"）——⚠️ 黑框那种 Mac 上复现不了，别当偶发 | `references/foundations.md`（焦点框 段） | **焦点框已由源头全局基线 `el-theme/base/focus.scss` 接管，下游一行都不用写**。黑框来自浏览器 UA 默认样式 `-webkit-focus-ring-color`（**Windows 解析为黑、macOS 为系统强调色**，故本机看不见）。⛔ **禁止**为消框而单写 `outline: none`（=用无障碍换观感，键盘用户失去焦点指示）；**禁止**自写 outline 颜色/粗细/偏移。组件特化只允许改 `outline-color`。**三个坑各有各的解，别混成一个「关掉 outline」**：① **输入框双层框**（内圈 box-shadow + 外圈 outline）——UA 规定支持键盘输入的元素**恒定匹配** `:focus-visible`，鼠标点的也命中，基线 `:focus{outline:none}` 压不住；源头已按类名豁免 `.el-input__inner`/`.el-textarea__inner`/`.el-select__input` 等，**下游什么都不用做**，更不许写裸 `input:focus-visible{outline:none}`（会让原生 input 失去全部焦点指示）。② **Esc / 关弹窗后凭空冒框**——是残留焦点被切进键盘模态，在退出交互里 `blur()` 断焦，不是去删 outline。③ ⚠️ EP 自带 `.el-button:focus-visible`(0,2,0) 特异性高于基线(0,1,0)，故 `button.scss` 那条覆盖**不是冗余、删了会静默退化成浅淡色** |
