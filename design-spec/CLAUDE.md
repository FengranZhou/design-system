...
5. **落地前自检** —— 每次改动完成前，逐条核对上述各点；命中「找不到令牌 / 找不到组件 / 想在使用方 scoped 覆盖组件外观 / 引用方接入方式不合规」时必须暂停并向用户说明，由用户决定新增全局定义还是调整方案。
   ...
   > **⭐ 样式值对账清单（页面 / 模块布局类改动必做，交付物而非自觉动作）**：凡新写或改动页面、模块布局（demo 页、典型页面、业务组件骨架、模式样式…），交付前必须**输出一张对账表**——本次新写的**每一处间距 / 圆角 / 字阶 / 颜色**逐条列出，标注它对应 `references/foundations.md`（间距为 `design-token/spacing-usage.ts`）里的哪个场景锚点。**标不出锚点的值不许静默交付**：要么升级为向用户提问（场景缺档），要么说明其合法出处（如 demo 展示脚手架的自有版式约定——注意 demo 版式与业务页锚点是**两个口径**，不得互相迁移）。
   >
