...
> 落地范围：指标条 `.iflyv-metric-item`（源头 `patterns/metric-strip.scss`）、图表环心数字（`Chart` 源头）等所有「大数字 + 下方说明」的组合。
...
| 场景 | 取值 | 为什么不是邻近档 |
|---|---|---|
| **工具栏内元素之间**（搜索框、筛选项、按钮…） | **不写**，由源头 `.iflyv-toolbar__left/__right` 的 `gap: spacing-3`（12）提供 | 曾有"12~16 自选"的表述，已作废——工具栏布局全在源头 `el-theme/patterns/toolbar.scss`，使用方写任何值（哪怕写对 12）都是私货 |
...
| `--iflyv-grid-columns` | 24 | 内容区栅格列数。**不直接引用**——用源头约定类 `.iflyv-grid` / `.iflyv-grid__col-*`（见下） |
| `--iflyv-grid-gutter` | 16px（= `spacing-4`） | 栅格列间水槽。与「卡片之间」同档，保证栅格并排的卡片与非栅格场景间距一致 |

> **栅格怎么用**：写 `<div class="iflyv-grid"><div class="iflyv-grid__col-16">…</div><div class="iflyv-grid__col-8">…</div></div>`，样式在源头 `el-theme/patterns/grid.scss`。**只提供 `col-6/8/10/12/14/16/18/20/24` 九档**——「模块宽度不得小于 6 列」由源头保证，写不出违规值。**不用 `el-row`/`el-col`**（EP 走 flex + 负 margin，`:gutter` 拿不到 CSS 变量）。完整规则见 `efficiency-guide.md`「栅格系统」。
...
**源头已按元素豁免，下游一行都不用写**：`base/focus.scss` 末段（cascader 节点 / tree 节点）、`components/dropdown.scss`（菜单项）、`components/tabs.scss`（tab 项）、`components/button.scss`「当 el-dropdown 触发器」段（**el-button 触发器**——按钮/纯图标/文字+箭头三形态通吃，替代指示 = 各形态 hover 同款反馈：次按钮灰底、primary/danger 换 hover 色、text 文字变色）、`components/PageFrame/PageFrame.vue`（头像下拉**触发器**——业务组件自己的 DOM，豁免写在组件内，替代指示 = 与右侧图标按钮同款的 hover/focus 底色）。
