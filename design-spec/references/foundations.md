...
| 令牌 | 值 | 应用场景 |
|---|---|---|
| `spacing-0_5` ~ `spacing-2` | 2 / 4 / 6 / 8px | 微间距。逐档：**2** 紧贴元素的视觉对齐补偿；**4** 控件内图标与其紧邻文字（按钮内 icon+文字）、卡内通栏出血面板距卡缘的细缝；**6** Tag / Badge 等小控件的左右内边距；**8** 列表项内子元素（标题与描述、主信息与副信息；含横向的图标/标签与其同行文字）、**常规标题与其下方内容之间**（与「标题与其描述」同档；下方是通栏/出血块时放宽一档取 `spacing-3`）。⚠️ **图标与图标之间不在这几档**——一排图标（无论是否可点）走 `spacing-4`（16px） |
