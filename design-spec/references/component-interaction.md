...
| **`el-calendar` 日历** | 未纳入。源头零适配（写出来是 EP 原生观感）。⚠️ 注意与**日期选择**区分：选一个日期 / 一段区间是 `el-date-picker` 的事，本组件是**整月网格视图**（课表、排课、日程铺陈） | 选日期一律用 `el-date-picker`（见 DatePicker 段）；确需月历视图（课表 / 排课）先与设计负责人确认——这类通常要按业务定制，不宜直接用 EP 原生 |
