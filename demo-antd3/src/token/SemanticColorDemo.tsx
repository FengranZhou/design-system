...
/**
 * Color 语义色板 —— 与 demo/src/components/token/SemanticColorDemo.vue 对位。
 * 色块的 hex 由运行时实测得出；能反查到基础色板色阶的额外标出来源（如 green-6）。
 */
...
export default function SemanticColorDemo() {
  const { rootRef, swatchValues, sourceLabels } = useTokenValues()

  return (
    <section className="demo-section demo-plain" ref={rootRef}>
      <h2 className="demo-section__title">Color 语义色板</h2>

      <div className="demo-block">
        {COLOR_GROUPS.map((group) => (
          <div key={group.label} className="token-group">
            <div className="token-group__label">{group.label}</div>
            <div className="token-color-grid">
              {group.tokens.map((t) => (
                <div
                  key={t.name}
                  className="token-color-item"
                ...
                >
                  ...
                  />
                  <span className="token-color-item__meta">
                    <span className="token-name">{t.name.replace('--iflyv-', '')}</span>
                    <span className="token-value">{swatchValues[t.name] || '…'}</span>
                    {(t.desc || sourceLabels[t.name]) && (
                      <span className="token-desc">
                        {t.desc}
                        {sourceLabels[t.name] && (
                          <span className="token-source">{sourceLabels[t.name]}</span>
                        )}
                      </span>
                    )}
                  </span>
